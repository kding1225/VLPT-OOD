...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        self.test_time_fuse = cfg.TRAINER.PROGRAD.TEST_TIME_FUSE
        self.test_time_weight = cfg.TRAINER.PROGRAD.TEST_TIME_WEIGHT
        if self.test_time_fuse:
            self.zs_weight = self.compute_zs_weights(cfg, classnames, clip_model)

    def compute_zs_weights(self, cfg, classnames, clip_model):
        temp = CUSTOM_TEMPLATES[cfg.DATASET.NAME]
        if isinstance(temp, dict):
            temp = temp[cfg.DATASET.SUBSAMPLE_TASKS]
        ...
        with torch.no_grad():
            text_features = clip_model.cuda().encode_text(prompts.long().cuda())
